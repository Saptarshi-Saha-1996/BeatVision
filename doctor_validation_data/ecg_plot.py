...
def _ax_plot(ax, x, y, secs=10, lwidth=0.5, amplitude_ecg = 1, time_ticks =0.2,grid = True): #lwidth=0.7
    ...
    if peaks is not None:
        ...
        if top_k is not None:
            ...
            for i in range(len(left_r_peak)):
                ax.add_patch(Rectangle( (left_r_peak[i],0),height=1.1,width=width[i],alpha=0.8,color='green'))
        ...
        if numbering and 'R' in peaks_index:
            r_peaks = peaks_index['R']
            for i in range(len(r_peaks) - 1):
                mid_point = (r_peaks[i] + r_peaks[i + 1]) / 2 / sample_rate
                ax.text(mid_point, -0.1, str(i + 1), color='blue', fontsize=8, ha='center', va='top')
